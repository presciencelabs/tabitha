...
**The fix requires the Workers Builds API directly -- there is no dashboard path to the non-production trigger's Build command/Variables.** With an API token scoped to `Workers Builds Configuration:Edit`, created as a **User** API Token (My Profile -> API Tokens) -- confirmed 2026-09-02 that the Workers Builds API rejects Account-Owned tokens outright with a generic `401 Invalid token` regardless of permissions granted (a known, currently-unsupported limitation, not a scoping mistake -- see the Cloudflare Community bug report linked from `tools/workers/.env`):
...
**Watch paths (`path_includes`) *are* enforced on the non-production trigger -- contrary to a long-standing belief on this repo, re-tested and disproven 2026-09-03.** The non-production trigger's Cloudflare default is `path_includes: ["*"]`, and because the dashboard exposes no watch-path control for that trigger at all, the observed symptom (every app preview-building on every PR regardless of the diff) was originally written up in [issue #74](https://github.com/presciencelabs/tabitha/issues/74) as an unfixable Cloudflare limitation -- watch paths supposedly being ignored outright for non-production branches. That conclusion rested on the Workers Builds API appearing unreachable, which was really the Account-Owned-token rejection described above. With a genuine **User** token, PATCHing `path_includes` onto a non-production trigger both succeeds *and* takes effect: scoping `ontology`'s trigger and then pushing a commit touching only `apps/www/` left `ontology` correctly unbuilt while every still-unscoped app built. `tools/workers` now applies each app's derived watch paths to both triggers, so a PR only previews the apps it actually touches. The trade-off, and the reason `.github/pull_request_template.md` calls it out: a PR touching only root docs, `.github/`, or tooling now produces **no** preview builds or preview URLs at all.
