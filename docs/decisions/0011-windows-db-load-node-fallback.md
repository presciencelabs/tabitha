...
Every bug this fix addresses was found by a human running these scripts on a real Windows machine, not by CI — the project's CI has never run on Windows. To catch the next one automatically, `windows_dx_smoke` (a `windows-latest` job running `pnpm db:load` and `pnpm check:doctor`) was added to `ci.yml`. It doesn't mirror the whole suite there (`windows-latest` runners bill at roughly 2x Linux minutes, and production is Cloudflare Workers — OS-agnostic, so a Windows job only ever protects local DX, not anything that ships) — it reuses the change-scoping mechanism from [ADR 0008](0008-ci-change-scoping.md) (`scripts/ci/plan.ts`'s `run_windows_smoke` output) so it only runs when a change actually touches `scripts/dx/**` or `tools/databases/**`. Advisory only (`continue-on-error: true`), surfaced in the same PR summary comment as the other Stage 2 checks, never blocking a merge on its own.
