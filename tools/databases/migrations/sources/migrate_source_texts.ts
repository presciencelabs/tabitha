...
export function migrate_source_texts(tabitha_sources_db: Database, tbta_sources_from_input: string[]) {
	console.log(`[Sources migration] Prepping Sources table in ${tabitha_sources_db.filename}...`)
	...
	tbta_sources_from_input.map(tbta_source_from_input => {
		console.log(`[Sources migration] Extracting relevant table names from ${tbta_source_from_input}...`)
		...
		tbta_source_tablenames.map(tbta_table_name => {
			...
			console.log()
			console.log(`[Sources migration] transforming data from ${tbta_table_name} and inserting into ${tabitha_sources_db.filename}:`)
			...
			console.log(`\n[Sources migration] ${valid_rows.length} rows inserted.`)
		})
	})
