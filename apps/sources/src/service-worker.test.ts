...
describe('service worker config (apps/sources/vite.config.js)', () => {
...
describe('compiled service worker (apps/sources/.svelte-kit/output/client/sw.js)', () => {
	...
	it('registers the /analyze runtime-caching rule as NetworkFirst, GET-only', () => {
		expect(compiled_sw).toMatch(/"\/analyze"===\w+\.pathname/)
		expect(compiled_sw).toContain('sources-analyze-api')
