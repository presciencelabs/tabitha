...
export async function run_check(text: string): Promise<CheckResponse> {
...
export function get_status(tokens: SimpleToken[]): CheckStatus {
...
export function expand_token(token: SimpleToken): SimpleToken[] {
	if (token.pairing) {
