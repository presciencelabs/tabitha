...
describe('parse', () => {
	...
	describe('problems detected', () => {
		test('bad_notesNotation[badbracket you', () => {
			...
			expect_error({ token: results[0], message: ERRORS.NO_SPACE_BEFORE_UNDERSCORE })
			expect(results[1].token).toBe('you')
			expect_error_to_match({ token: results[1], regex: /^Second person pronouns/ })
			expect(results[2].token).toBe('.')
			expect_error({ token: results[2], message: ERRORS.MISSING_PERIOD })
			expect(results[3].token).toBe(']')
			expect_error({ token: results[3], message: ERRORS.MISSING_CLOSING_BRACKET })
		...
		test('Ok _notesNotation text[badbracket you', () => {
			...
			expect_error({ token: results[2], message: ERRORS.NO_SPACE_BEFORE_OPENING_BRACKET })
			expect(results[3].token).toBe('badbracket')
			expect_no_message(results[3])
			expect(results[4].token).toBe('you')
			expect_error_to_match({ token: results[4], regex: /^Second person pronouns/ })
			expect(results[5].token).toBe('.')
			expect_error({ token: results[5], message: ERRORS.MISSING_PERIOD })
			expect(results[6].token).toBe(']')
			expect_error({ token: results[6], message: ERRORS.MISSING_CLOSING_BRACKET })
			expect(results).length(7)
		})
		...
		test('Ok _notesNotation [fixedbracket you(Paul', () => {
			...
			expect_error({ token: results[4], message: ERRORS.MISSING_CLOSING_PAREN })
			expect(results[5].token).toBe('.')
			expect_error({ token: results[5], message: ERRORS.MISSING_PERIOD })
			expect(results[6].token).toBe(']')
			expect_error({ token: results[6], message: ERRORS.MISSING_CLOSING_BRACKET })
		...
		test('Paul explained [the Christ needed to become alive again]].', () => {
			...
			expect(results[0].token).toBe('[')
			expect_error({ token: results[0], message: ERRORS.MISSING_OPENING_BRACKET })
			expect(results[1].token).toBe('Paul')
			expect_no_message(results[1])
			expect(results[2].token).toBe('explained')
			expect_no_message(results[2])
		...
		test('John said, ["what do you(person) want?" then that person took the book that John had]', () => {
			...
			expect_error({ token: results[5], message: ERRORS.FIRST_WORD_NOT_CAPITALIZED })
			...
			expect(results[10].messages.length).toBe(0)
			expect(results[11].token).toBe(']')
			expect_error({ token: results[11], message: ERRORS.MISSING_CLOSING_BRACKET })
			expect(results[12].token).toBe('[')
			expect_error({ token: results[12], message: ERRORS.MISSING_OPENING_BRACKET })
			expect(results[13].token).toBe('then')
			expect_error({ token: results[13], message: ERRORS.FIRST_WORD_NOT_CAPITALIZED })
			...
			expect_error({ token: results[23], message: ERRORS.MISSING_PERIOD })
			expect(results).length(24)
		})
	})
})
