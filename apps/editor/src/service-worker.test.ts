...
describe('service worker config (apps/editor/vite.config.js)', () => {
...
describe('compiled service worker (apps/editor/.svelte-kit/output/client/sw.js)', () => {
	...
	it('registers the /check runtime-caching rule as NetworkFirst, GET-only', () => {
		expect(compiled_sw).toMatch(/"\/check"===\w+\.pathname/)
		expect(compiled_sw).toContain('editor-check-api')
